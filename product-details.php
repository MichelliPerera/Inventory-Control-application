...
// Add product to cart if action is set to "add"
if(isset($_GET['action']) && $_GET['action']=="add"){
	...
	if(isset($_SESSION['cart'][$id])){
		...
		// If product is not in cart, fetch product details from the database
		$sql_p="SELECT * FROM products WHERE id={$id}";
		$query_p=mysqli_query($con,$sql_p);
		// If product exists, add it to the cart session variable
		if(mysqli_num_rows($query_p)!=0){
			$row_p=mysqli_fetch_array($query_p);
			$_SESSION['cart'][$row_p['id']]=array("quantity" => 1, "price" => $row_p['productPrice']);
			// Display alert message and redirect to cart page
			echo "<script>alert('Product has been added to the cart')</script>";
			echo "<script type='text/javascript'> document.location ='my-cart.php'; </script>";
		}else{
			$message="Product ID is invalid"; // Display error message if product ID is invalid
		}
	}
}
...
// Add product to wishlist if action is set to "wishlist"
if(isset($_GET['pid']) && $_GET['action']=="wishlist" ){
	...
	{
		// Add product to wishlist table in the database
		mysqli_query($con,"insert into wishlist(userId,productId) values('".$_SESSION['id']."','$pid')");
		// Display alert message and redirect to wishlist page
		echo "<script>alert('Product added to wishlist');</script>";
		header('location:my-wishlist.php');
	}
}

// Add product reviews if form is submitted
if(isset($_POST['submit']))
{
	$qty=$_POST['quality']; // Get quality from form
	$price=$_POST['price']; // Get price from form
	$value=$_POST['value']; // Get value from form
	$name=$_POST['name']; // Get name from form
	$summary=$_POST['summary']; // Get summary from form
	$review=$_POST['review']; // Get review from form
	// Insert product review into productreviews table in the database
...
<!-- Fetch category, subcategory, and product name based on product ID-->
